          ...
          return (
            <div key={i}>
              <NavLink className="listCat" to={`/shop/${catName}`}>
                <h4 className={currSub === catName ? 'selectedCat' : 'text-dark'}>
                  {catName}
                </h4>
              </NavLink>
              {currSub === catName && subCategories.length > 0 && (
                <div className="subCategories">
                  {subCategories.map((subCat, index) => (
                    <NavLink key={index} className="listSubCat d-flex" to={`/shop/${catName}/${subCat}`} style={subcategory === subCat ? {color: '#088178'} : {}}>
                      {subcategory === subCat ? <i className="fa-solid fa-check me-1"></i> : ''} <h6>{subCat}</h6>
                    </NavLink>
                  ))}
                </div>
              )}
